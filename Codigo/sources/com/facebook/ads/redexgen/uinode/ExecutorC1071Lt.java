...
public final class ExecutorC1071Lt implements Executor {
    ...
    static {
        A04();
        A08 = new ExecutorC1071Lt(A00(7, 7, 8), 0);
        A06 = new ExecutorC1071Lt(A00(0, 5, 91), 0);
        A07 = new ExecutorC1071Lt(A00(5, 2, 3), 0);
        A03 = 32;
        A0C = Executors.newSingleThreadExecutor();
        A0B = Executors.newFixedThreadPool(5);
        A0D = new AtomicBoolean();
        A0E = new AtomicBoolean();
        A0A = Executors.newCachedThreadPool(new ThreadFactoryC1069Lr());
        A09 = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue());
    }

    public ExecutorC1071Lt(String str, int i10) {
        this.A02 = i10;
        int CPU_COUNT = Runtime.getRuntime().availableProcessors();
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(Math.max(2, Math.min(CPU_COUNT - 1, 4)), (CPU_COUNT * 2) + 1, 30L, TimeUnit.SECONDS, new LinkedBlockingQueue(), new ThreadFactoryC1070Ls(this, str));
    ...
    public static void A05(Context context) {
        A0D.set(C0988Ih.A2K(context));
        A0E.set(C0988Ih.A2J(context));
    }
