...
public abstract class AbstractC0844Cb {
    ...
    public static C0843Ca A00(byte[] bArr) {
        C0980Hz c0980Hz = new C0980Hz(bArr);
        if (c0980Hz.A07() < 32) {
            return null;
        }
        c0980Hz.A0Y(0);
        if (c0980Hz.A08() != c0980Hz.A04() + 4 || c0980Hz.A08() != CJ.A0s) {
            return null;
        }
        int dataSize = CJ.A01(c0980Hz.A08());
        if (dataSize > 1) {
            Log.w(A01(0, 12, 98), A01(12, 26, 36) + dataSize);
            return null;
        }
        UUID uuid = new UUID(c0980Hz.A0L(), c0980Hz.A0L());
        if (dataSize == 1) {
            int atomType = c0980Hz.A0H() * 16;
            if (A01[0].length() == 21) {
            ...
            c0980Hz.A0Z(atomType);
        }
        int atomSize = c0980Hz.A0H();
        if (atomSize != c0980Hz.A04()) {
            return null;
        }
        byte[] bArr2 = new byte[atomSize];
        c0980Hz.A0c(bArr2, 0, atomSize);
        return new C0843Ca(uuid, dataSize, bArr2);
    }

    public static UUID A02(byte[] bArr) {
        UUID uuid;
        C0843Ca parsedAtom = A00(bArr);
        if (parsedAtom != null) {
            uuid = parsedAtom.A01;
            return uuid;
        }
